...
type Client = {
    ...
    member this.GalleryFolderId 
            ...
            : Async<Result<GalleryFolderId.Response, ErrorClient>> =
        job {
            match parameters.Validate() with
            | Error errors ->
                return
            ...
            | Ok validatedParameters ->
                let request : TRequest =
                    ...
                    |> Client.AddQueryString "mature_content" (string validatedParameters.MatureContent)
                let! (json : Result<string, ErrorClient>) = this.RunRequestJob request
                let folderId : Result<GalleryFolderId.Response, ErrorClient> =
    ...
    member this.GalleryFolders 
            ...
            : Async<Result<GalleryFolders.Response, ErrorClient>> =
        job {
            match parameters.Validate() with
            | Error errors ->
                return
            ...
            | Ok validatedParameters ->
                let request : TRequest =
                    ...
                    |> Client.AddQueryString "mature_content" (string validatedParameters.MatureContent)
                let! (json : Result<string, ErrorClient>) = this.RunRequestJob request
                let folders : Result<GalleryFolders.Response, ErrorClient> =
                    Result.bind (Json.deserializeEx<GalleryFolders.Response> S.jsonConfig >> Ok) json
                return folders
        } |> Job.toAsync

    member this.UserFriends
            (parameters: UserFriends.Parameters)
            : Async<Result<UserFriends.Response, ErrorClient>> =
        job {
            match parameters.Validate() with
            | Error errors ->
                return
            ...
            | Ok validatedParameters ->
                let request : TRequest =
                    this.CreateRequest (this.Endpoints.UserFriends validatedParameters.Username)
                    |> Client.AddOptionalQueryString "offset" (Option.map string validatedParameters.Offset)
                    |> Client.AddOptionalQueryString "limit" (Option.map string validatedParameters.Limit)
                    |> Client.AddQueryString "mature_content" (string validatedParameters.MatureContent)
                let! (json : Result<string, ErrorClient>) = this.RunRequestJob request
                let friends : Result<UserFriends.Response, ErrorClient> =
    ...
    member this.UserStatuses
            (parameters: UserStatuses.Parameters)
            : Async<Result<UserStatuses.Response, ErrorClient>> =
        job {
            match parameters.Validate() with
            | Error errors ->
                return
            ...
            | Ok validatedParameters ->
                let request : TRequest =
                    this.CreateRequest this.Endpoints.UserStatuses
                    |> Client.AddQueryString "username" validatedParameters.Username
                    |> Client.AddOptionalQueryString "offset" (Option.map string validatedParameters.Offset)
                    |> Client.AddOptionalQueryString "limit" (Option.map string validatedParameters.Limit)
                    |> Client.AddQueryString "mature_content" (string validatedParameters.MatureContent)
                let! (json : Result<string, ErrorClient>) = this.RunRequestJob request
                let statuses : Result<UserStatuses.Response, ErrorClient> =
                    Result.bind (Json.deserializeEx<UserStatuses.Response> S.jsonConfig >> Ok) json
                return statuses
        } |> Job.toAsync
